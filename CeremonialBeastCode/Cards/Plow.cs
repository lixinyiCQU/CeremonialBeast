...
public sealed class Plow : CeremonialBeastCard
{
    ...
    protected override async Task OnPlayCard(PlayerChoiceContext choiceContext, CardPlay cardPlay)
    {
        ...
        bool facingRight = (Owner.Creature.GetCreatureNode()?.Body.Scale.X ?? -1f) < 0f;

        NCombatRoom.Instance?.CombatVfxContainer.AddChildSafely(
            NHorizontalLinesVfx.Create(new Color("BFFFC880"), 1.2, movingRightwards: facingRight));

        await Cmd.Wait(0.5f);

        NCombatRoom.Instance?.RadialBlur(facingRight ? VfxPosition.Right : VfxPosition.Left);
    ...
    public static async Task<IEnumerable<CardModel>> CreateInHand(Player owner, int count, ICombatState combatState, bool upgraded = false)
    {
        ...
        {
            CardModel newCard = combatState.CreateCard<Plow>(owner);
