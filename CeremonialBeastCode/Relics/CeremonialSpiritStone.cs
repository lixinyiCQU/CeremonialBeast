...
public sealed class CeremonialSpiritStone : CeremonialBeastRelic
{
    public override RelicRarity Rarity => RelicRarity.Starter;

    protected override IEnumerable<DynamicVar> CanonicalVars => [new DynamicVar("CardsCount", 1m)];

    protected override IEnumerable<IHoverTip> ExtraHoverTips => HoverTipFactory.FromCardWithCardHoverTips<SacrificialStone>();

    // ✨ 在这里告诉引擎升级后的遗物是谁
    public override RelicModel? GetUpgradeReplacement()
    {
        return ModelDb.Relic<SacredCeremonialCrystal>().ToMutable();
    }
    ...
    public override async Task BeforeHandDraw(Player player, PlayerChoiceContext choiceContext, ICombatState combatState)
    {
        ...
        {
            Flash();
            CardModel cardToAdd = ModelDb.Card<SacrificialStone>().ToMutable();
